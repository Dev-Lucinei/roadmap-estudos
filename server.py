...
class RoadmapHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def list_roadmaps(self):
        ...
        files = [f for f in os.listdir(DATA_DIR) if f.endswith('.json') and f not in EXCLUDED]
        self.send_response(200)
    ...
    def handle_diagnosis(self, data):
        try:
            ...
            self.send_header('Content-Type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({"status": "error", "message": f"Erro no diagnóstico: {str(e)}"}).encode('utf-8'))
