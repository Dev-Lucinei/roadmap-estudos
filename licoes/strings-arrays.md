...
[
  {
    "question": "Se você tem um array de objetos 'usuarios' e precisa obter um novo array contendo apenas os nomes dos usuários que estão ativos, qual a combinação mais semântica de métodos?",
    "options": [
      "usuarios.forEach().push()",
      "usuarios.filter(u => u.ativo).map(u => u.nome)",
      "usuarios.reduce((acc, u) => u.nome, [])",
      "usuarios.find(u => u.ativo).split('')"
    ],
    "answer": 1
  },
  {
    "question": "Qual a principal diferença entre os métodos .slice() e .splice() em arrays JavaScript?",
    "options": [
      ".slice() remove elementos e .splice() apenas copia.",
      "Ambos fazem a mesma coisa, mas .slice() é mais rápido.",
      ".slice() retorna uma cópia de parte do array (imutável), enquanto .splice() altera o array original (mutável).",
      ".splice() só funciona em Strings."
    ],
    "answer": 2
  },
  {
    "question": "Um desenvolvedor precisa transformar a string 'Engenharia de Software' no array ['Engenharia', 'Software'] removendo o 'de'. Qual abordagem é a mais eficiente?",
    "options": [
      "string.split(' ').filter(word => word !== 'de')",
      "string.replace('de', '').split(' ')",
      "string.substring(0, 10).split()",
      "Array.from(string).pop('de')"
    ],
    "answer": 0
  }
]
