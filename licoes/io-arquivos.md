...
[
  {
    "question": "Qual das alternativas abaixo garante escrita atômica de um arquivo em Node.js?",
    "options": [
      "Usar `fs.writeFileSync` diretamente no caminho final",
      "Escrever em um arquivo temporário e depois renomear com `fs.rename`",
      "Utilizar `fs.appendFile` em vez de `fs.writeFile`",
      "Abrir o arquivo com a flag `a+`"
    ],
    "answer": 1
  },
  {
    "question": "Em Python, qual módulo fornece a classe `Path` para manipulação de caminhos de forma portátil?",
    "options": [
      "os",
      "sys",
      "pathlib",
  ...
  {
    "question": "Qual API do navegador permite gerar um URL temporário para download de um Blob?",
    "options": [
